...
export interface AvailableAssessment {
  ...
  tags: string[];
  category: AssessmentCategory; // Updated to use the proper type
...
export interface AssessmentDetail extends AvailableAssessment {
  ...
  categories: AssessmentCategoryDetail[]; // Breakdown by category/section
...
export const getDifficultyText = (
  difficulty: AvailableAssessment["difficulty"]
) => {
  switch (difficulty) {
    case "easy":
      return "易しい";
