...
export const mockAvailableAssessments: AvailableAssessment[] = [
  {
    ...
    title: "React基礎スキル診断",
    description:
      "Reactの基本的な概念とフックに関する理解度を測定します。コンポーネント、ステート、プロップスなどが含まれます。",
    ...
    category: "technical", // Updated to use proper category
  ...
  },
  {
    ...
    description:
      "チームを率いる上で必要なコンピテンシー（意思決定、コミュニケーション、動機付けなど）を評価します。",
    ...
    category: "personality", // Updated to use proper category
  ...
  },
  {
    ...
    description:
      "組織全体のエンゲージメントレベル、満足度、改善点を把握するための匿名サーベイです。",
  ...
  },
  {
    ...
    description:
      "Pandas, NumPy を用いた基本的なデータ処理、可視化能力をテストします。",
  ...
  },
  {
    ...
    description:
      "プロジェクトの計画、実行、監視、終結に関する基本的な知識を問います。",
    ...
    category: "others", // Updated to use proper category
  ...
  },
  {
    ...
    description:
      "ジェネリクス、条件型、Mapped Typesなど、TypeScriptの高度な型機能に関する理解を深掘りします。",
...
// Function to simulate API call for getting available assessments
export const getAvailableAssessments = async (): Promise<
  AvailableAssessment[]
> => {
  // Simulate network delay
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(mockAvailableAssessments);
    }, 100);
  });
};
